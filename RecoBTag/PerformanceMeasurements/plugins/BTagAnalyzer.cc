  ...
  CentralityBinTag_(consumes<int>(iConfig.getParameter<edm::InputTag>("CentralityBinSrc")))
{
  ...
  // cout << "JetCollectionTag_ = " << JetCollectionTag_.c_str() << endl;
  //cout << "SubJetLabels = " << SubJetLabels_.c_str() << endl;
  ...
  if( runSubJets_ && ( SubJetCollectionTags_.size()>0 || SubJetLabels_.size()>0 ) )
  {
    ...
    cout << "SubJetCollectionTags_.size() = " << SubJetCollectionTags_.size() << endl;
      cout << "SubJetLabels_.size() = " << SubJetLabels_.size() << endl;
  ...
  if ( runSubJets_ ) {
    ...
    //    cout << "IK::JetInfo::runSubJets_ = " << runSubJets_ << endl;
    //cout << "IK:JetInfo::SubJetLabels_.size() = " << SubJetLabels_.size() << endl;
...
void BTagAnalyzerT<IPTI,VTX>::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  if( !useSelectedTracks_ ){
    fillHelpers(iEvent);
    cout << "no selected tracks: fillHelpers(iEvent)" << endl;
  }
  ...
  if (runSubJets_)
  {
    ...
    cout << "resize subjets using SubJetLabels_.size() = " << SubJetLabels_.size() << endl;

    subjetColls.resize( SubJetLabels_.size() );
    for ( size_t i = 0; i < SubJetLabels_.size(); ++i ){
      iEvent.getByToken(SubJetCollectionTags_[i], subjetColls[i]);

      //      cout << "IK:SubJetLabels_.at(i) = " << SubJetLabels_[i] << endl;
      //cout << "IK:got event by token" << endl;
...
// cout << " B hadron " << EventInfo.nBHadrons << "  id " << EventInfo.BHadron_pdgID[EventInfo.nBHadrons] << "  pT " << EventInfo.BHadron_pT[EventInfo.nBHadrons] << "  eta " << EventInfo.BHadron_eta[EventInfo.nBHadrons]<< "  phi " << EventInfo.BHadron_phi[EventInfo.nBHadrons] << "  ->D " << idD1 << " " << idD2 << "  nCh " << nCharged << endl;
  ...
  cout << "before iSetup.get<JetTagComputerRecord>().get( SVComputer_.c_str(), computerHandle );" << endl;
  cout << "SVComputer_.c_str() = " << SVComputer_.c_str() << endl;

  iSetup.get<JetTagComputerRecord>().get( SVComputer_.c_str(), computerHandle );

  cout << "after iSetup.get<JetTagComputerRecord>().get( SVComputer_.c_str(), computerHandle );" << endl;

  computer = dynamic_cast<const GenericMVAJetTagComputer*>( computerHandle.product() );

  //IK:
  if(!computer) cout << "IK::: NO computer!!!!" << endl;
  ...
  cout << "before processJets" << endl;
  //  cout << "test INNA : set iJetColl = 1" << endl;
  //iJetColl = 1;

  processJets(jetsColl, jetsColl, subjetColls, iEvent, iSetup, iJetColl); // the second 'jetsColl' is a dummy input here

  cout << "after processJets" << endl;

  cout << "IK::DO jets" << endl;
  //  cout << "IK::SVComputerSubJets_.c_str() = " << SVComputerSubJets_.c_str() << endl;
  cout << "IK::SVComputer_.c_str() = " << SVComputer_.c_str() << endl;
  //cout << "IK::runSubJets_ = " << runSubJets_ << endl;

  if (runSubJets_) {
    // for fat jets we might have a different jet tag computer
    // if so, grab the fat jet tag computer

    //cout << "runSubJets_!" << endl;
    //cout << "SVComputerSubJets_.c_str() = " << SVComputerSubJets_.c_str() << endl;
    //cout << "SVComputer_.c_str() = " << SVComputer_.c_str() << endl;
    ...
    for ( size_t i = 0; i < SubJetLabels_.size(); ++i )
    {
      cout << "inside for ( size_t i = 0; i < SubJetLabels_.size(); ++i )" << endl;
      iJetColl = 1+i ;
      cout << "iJetColl = " << iJetColl << endl;
					  ...
					  const edm::Event& iEvent, const edm::EventSetup& iSetup, const int iJetColl)
{
  ...
  cout << "IK::processJets" << endl;
  //  cout << "runSubJets_ = " << runSubJets_ << endl;
  //cout << "iJetColl = " << iJetColl << endl;

  if ( runFatJets_ && runSubJets_ && iJetColl == 0 )
  {

    //cout << "runFatJets_ && runSubJets_ && iJetColl == 0 :: HERE " << endl;
    for ( size_t i = 0; i < SubJetLabels_.size(); ++i )
  ...
  if ( runSubJets_ && iJetColl > 0 )
  {
    ...
    //cout << "runSubJets_ && iJetColl > 0 :: HERE " << endl;
  ...
  for ( PatJetCollection::const_iterator pjet = jetsColl->begin(); pjet != jetsColl->end(); ++pjet ) {
    ...
    // cout << "IK: number of subjets collections = " << pjet->nSubjetCollections() << endl;
    //    cout << "IK: name of 1st subjet collection = " << pjet->subjetCollectionNames().at(0) << endl;
    ...
    if( runSubJets_ && iJetColl > 0 )
    {
      ...
      //cout << "IK::runSubJets_ && iJetColl > 0" << endl;
	...
	cout << "number of subjets = " << (int)subjets.size()<< endl;
    ...
    if ( runFatJets_ && iJetColl == 0 )
    {
      ...
      */
      ...
      /*
      JetInfo[iJetColl].Jet_tau1[JetInfo[iJetColl].nJet] = pjet->userFloat("akCsSoftDrop4PFNjettiness:tau1");
      JetInfo[iJetColl].Jet_tau2[JetInfo[iJetColl].nJet] = pjet->userFloat("akCsSoftDrop4PFNjettiness:tau2");
      */

      // SoftDrop kinematics

      /*
      JetInfo[iJetColl].Jet_ptSoftDrop[JetInfo[iJetColl].nJet]    = pjet->userFloat("akCsSoftDrop4PFSoftDrop:Pt");
      JetInfo[iJetColl].Jet_etaSoftDrop[JetInfo[iJetColl].nJet]   = pjet->userFloat("akCsSoftDrop4PFSoftDrop:Eta");
      JetInfo[iJetColl].Jet_phiSoftDrop[JetInfo[iJetColl].nJet]   = pjet->userFloat("akCsSoftDrop4PFSoftDrop:Phi");
      JetInfo[iJetColl].Jet_massSoftDrop[JetInfo[iJetColl].nJet]  = pjet->userFloat("akCsSoftDrop4PFSoftDrop:Mass");
      JetInfo[iJetColl].Jet_jecF0SoftDrop[JetInfo[iJetColl].nJet] = pjet->userFloat("akCsSoftDrop4PFSoftDrop:jecFactor0");
      // Pruned kinematics
      JetInfo[iJetColl].Jet_ptPruned[JetInfo[iJetColl].nJet]    = pjet->userFloat("akCsSoftDrop4PFPruned:Pt");
      JetInfo[iJetColl].Jet_etaPruned[JetInfo[iJetColl].nJet]   = pjet->userFloat("akCsSoftDrop4PFPruned:Eta");
      JetInfo[iJetColl].Jet_phiPruned[JetInfo[iJetColl].nJet]   = pjet->userFloat("akCsSoftDrop4PFPruned:Phi");
      JetInfo[iJetColl].Jet_massPruned[JetInfo[iJetColl].nJet]  = pjet->userFloat("akCsSoftDrop4PFPruned:Mass");
      JetInfo[iJetColl].Jet_jecF0Pruned[JetInfo[iJetColl].nJet] = pjet->userFloat("akCsSoftDrop4PFPruned:jecFactor0");
      */
    }
    ...
    if ( runFatJets_ && runSubJets_ && iJetColl == 0 )
    {
      ...
      //      cout << "IK:runFatJets_ && runSubJets_ && iJetColl == 0" << endl;
      //cout << "SubJetLabels_.size() = " <<  SubJetLabels_.size() << endl;
	...
	cout << "(int)subjets.size() = " << (int)subjets.size() << endl;
    ...
    //    cout<<"taginfo labels"<<endl;
    //    for (auto x:pjet->tagInfoLabels())
    //      cout<<x<<endl;


    cout << "IK::ipTagInfos_ = " <<  ipTagInfos_ << endl;
    if(pjet->hasTagInfo(ipTagInfos_.c_str())) cout << "has tag info" << endl;
    else cout << "NO has tag info" <<endl;
    ...
    cout<<"TagInfo pointers : "<<ipTagInfos_<<" "<<ipTagInfo<<","<<svTagInfos_<<" "<<svTagInfo<<","<<svNegTagInfos_<<" "<<svNegTagInfo<<endl;
    ...
    //IK change it wrt https://github.com/cms-btv-pog/RecoBTag-PerformanceMeasurements/blob/7_5_X/plugins/BTagAnalyzer.cc#L1809
    //    if (useSelectedTracks_) ntagtracks = 0;//??????????????selectedTracks.size();

    cout << "IK: before selected tracks" << endl;

    cout << "IK:: selectedTracks.size()  = " <<   selectedTracks.size()<< " and trackSize = " << tracks.size() << endl;

    //    cout << "useSelectedTracks_ and ntagtracks = " << selectedTracks.size() << endl;
    //    if (useSelectedTracks_) ntagtracks = selectedTracks.size();
    if (useSelectedTracks_) ntagtracks = selectedTracks.size();
    else {
      ntagtracks = tracks.size();
      cout << "no sel tracks and ntagtracks " << ntagtracks << endl;
    }

    std::cout << "BTagAnalyser:: ntagtracks = " << ntagtracks << std::endl;
    ...
    cout << " JetInfo[iJetColl].Jet_nFirstTrack[JetInfo[iJetColl].nJet] = " <<  JetInfo[iJetColl].Jet_nFirstTrack[JetInfo[iJetColl].nJet] << endl;
    cout << "JetInfo[iJetColl].Jet_nFirstTrkInc[JetInfo[iJetColl].nJet] = " << JetInfo[iJetColl].Jet_nFirstTrkInc[JetInfo[iJetColl].nJet] << endl;
    cout << "(long)ipTagInfo = " << (long)ipTagInfo << endl;

    unsigned int trackSize = (long)ipTagInfo==0 ? 0 :selectedTracks.size();

    cout << "trackSize = " << trackSize << endl;
    ...
    for (unsigned int itt=0; itt < trackSize; ++itt)
    {
	  ...
	  cout << "IK:ipTagInfo->impactParameterData()[itt].ip2d.value() = " << ipTagInfo->impactParameterData()[itt].ip2d.value() << endl;
    ...
    for (size_t leptIdx = 0; leptIdx < (size_t)nSM; ++leptIdx) {
      ...
      JetInfo[iJetColl].PFMuon_IPsig[JetInfo[iJetColl].nPFMuon]        = 0;//(softPFMuTagInfo->properties(leptIdx).sip3dsig);
      JetInfo[iJetColl].PFMuon_IP2Dsig[JetInfo[iJetColl].nPFMuon]      = 0;//(softPFMuTagInfo->properties(leptIdx).sip2dsig);
    ...
    if ( storeCSVTagVariables )
    {
      ...
      cout << "storeCSVTagVariables" << endl;

      std::vector<const reco::BaseTagInfo*>  baseTagInfos;

      cout << "baseTagInfos" << endl;
      ...
      cout << "ipTagInfo = " << ipTagInfos_.c_str() << endl;
      cout << "svTagInfo = " << svTagInfos_.c_str() << endl;

      reco::TaggingVariableList vars = computer->taggingVariables(helper);

      cout << "after computer->taggingVariables(helper);" << endl;
      ...
      std::cout << "BTagAnalyser:: CSV jetNTracks = " << tagValList.size() << std::endl;
      ...
      std::cout << "after if(tagValList.size()>0) std::copy( tagValList.begin(), tagValList.end(), &JetInfo[iJetColl].TagVarCSV_trackEtaRel[JetInfo[iJetColl].nTrkEtaRelTagVarCSV] );" << std::endl;
      ...
      std::cout << "end of storeCSVTagVariables" << std::endl;
   }
    ...
    std::cout << "BEFORE std::vector<reco::btag::TrackIPData>  ipdata = ipTagInfo->impactParameterData();" << std::endl;

    std::vector<reco::btag::TrackIPData>  ipdata = ipTagInfo->impactParameterData();
    std::vector<std::size_t> indexes( sortedIndexes(ipdata) );

    std::cout << "after std::vector<reco::btag::TrackIPData>  ipdata = ipTagInfo->impactParameterData();" << std::endl;
    ...
    if ( useTrackHistory_ && indexes.size() != 0 && !isData_ ) {
      ...
      std::cout << "in useTrackHistory_ && indexes.size() != 0 && !isData_ " << std::endl;
    ...
    std::cout << "BEFORE VTXmap" << std::endl;
    ...
    std::cout << "BEFORE the loop: vtx < (size_t)svTagInfo->nVertices();" << std::endl;

    for (size_t vtx = 0; vtx < (size_t)svTagInfo->nVertices(); ++vtx)
      {

	std::cout << "inside the loop: vtx < (size_t)svTagInfo->nVertices();" << std::endl;

	JetInfo[iJetColl].SV_x[JetInfo[iJetColl].nSV]    = position(svTagInfo->secondaryVertex(vtx)).x();

	std::cout << "position(svTagInfo->secondaryVertex(vtx)).y() = " << position(svTagInfo->secondaryVertex(vtx)).y() << std::endl;
	...
	std::cout << "vertex.p4().pt() = " <<  vertex.p4().pt() << std::endl;
    ...
    std::cout << "JetInfo[iJetColl].nSV = " <<  JetInfo[iJetColl].nSV << std::endl;
    std::cout << "VTXmap.size() = " <<  VTXmap.size() << std::endl;
    ...
    if ( runFatJets_ && iJetColl == 0 )
    {
      ...
      std::cout << "BDTG_SV = " <<  BDTG_SV << std::endl;
    ...
    std::cout << "after svxPostracks " << std::endl;
    ...
    std::cout << "after svxNegtracks " << std::endl;
...
{
  cout<<"GettingTrack ... "<<tagInfos<<" has it? "<<jet.hasTagInfo(tagInfos)<<" no label "<<jet.tagInfoTrackIP()<<endl;
  cout << "IK::tagInfos.c_str() = " << tagInfos.c_str() << endl;
...
{
  cout<<"GettingCand ... "<<tagInfos<<" has it? "<<jet.hasTagInfo(tagInfos)<<" no label Track"<<jet.tagInfoTrackIP()<<endl;
  cout << "IK::tagInfos.c_str() = "<< tagInfos.c_str() << endl;
...
{
  //  cout<<"Getting vertex info ... "<<tagInfos<<" "<<jet.tagInfoSecondaryVertex(tagInfos.c_str())<<endl;
...
{
  //  cout<<"Getting vertex info2 ... "<<tagInfos<<" "<<jet.tagInfoCandSecondaryVertex(tagInfos.c_str())<<endl;
